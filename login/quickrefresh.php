<?php
	...
	// $isEmbed = false;
	// if (substr_count($_SERVER['HTTP_HOST'], "embed.") > 0 || isset($_GET["embed"])) $isEmbed = true;
	if (empty($embedded)) {
		$embedded = false;
	...
	try {
		...
		// else {
		if (!$embedded && !empty($bucket) && $bucket != "media" && !isset($_COOKIE[$bucket . "_suggs"])) {
			$updateSuggestions = true;
		...
		if (empty($_COOKIE["reddit_token"])) {
			...
			if (!$embedded) echo " done.</h1>";

			if ($updateUpvotes) {
				if (!$embedded) echo "<h1>Getting upvotes.";
	...
	if ($updateUpvotes && !$embedded) echo " done!</h1>";

	if ($updateSuggestions && !$embedded) {
	...
	if (!$embedded) { // && !isset($_GET["debug"])
		echo "<h1>Sending you back..</h1></div><script>setTimeout(function() { window.location.href = '" . str_replace("?debug", "", str_replace("//r/", "/r/", $_SERVER['REQUEST_URI'])) . "'; }, 2000);</script>";
		die();
	}
