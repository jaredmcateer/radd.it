...
<?php
    function nSum($str) {
		...
		$superSpecials = array(
			12, 13, 15, 16, 19, 21, 24, 31, 37, 41, 42, 51, 61, 73, 91
			...
			, 511, 522, 555, 557, 599
			, 611, 616, 619, 622, 636, 655, 666, 691
			, 711, 722, 777, 799
			, 811, 815, 822, 888, 899
			, 911, 916, 922, 988, 999

			, 1211, 1216, 1219, 12124, 13114, 14419 
		...
		$specials = array_merge(
			array(3, 7, 9, 12, 14, 23, 32, 51, 82, 73, 99, 123, 125, 317, 717, 811, 927, 928)
			, $superSpecials
		);
        ...
        /*
            DONE IF...
                ...
                ..master + digit
        */
        if ($cnt == 1 || in_array($str, $masters) || in_array($str, $specials)) return $str;
        ...
        if ($cnt == 2) $results[] = $nums[0] + $nums[1];
        	...
        	// add all
            $results[] = $nums[0] + $nums[1] + $nums[2] + $nums[3] + $nums[4] + $nums[5];
            // add first and second
            $results[] = ($nums[0] + $nums[1]) . $nums[2] . $nums[3] . $nums[4] . $nums[5];
            // add second and third
            $results[] = $nums[0] . ($nums[1] + $nums[2]) . $nums[3] . $nums[4] . $nums[5];
        ...
        $weights = array(
            ...
            , $specials
            , array("9", "29", "38", "47", "56", "65", "74", "83", "92", "101", "128", "129", "137", "173", "218", "281", "416", "461", "623", "632")   // 11s
...
//	else if ($mode == "report") echo "nada.<br>";
	...
	$specials = array(3, 7, 9, 12, 14, 23, 32, 51, 82, 73, 99, 123, 125, 317, 717, 811, 927, 928);
	...
	while ($ev = $baseEvents->fetch_row()) {
        ...
        // keep track of reasons to later use for weighting
        $becauseReasons[] = $reason;
   ...
   //  	if (strlen(str_replace($masters, "", $str)) == strlen($str) - 2)
    	...
    	$numbers = preg_replace("/[^1-9 ]/", "", $descrip);
	...
	foreach ($becauseReasons as $k => $reasons) {
		$reas = explode("<br>", substr($reasons, 4));

		foreach ($reas as $k2 => $r) {
			$r = preg_replace("/[^1-9]/", "", $r);
			if ($r == "") continue;
		...
		for ( ; $month <= 12 ; $month++) { 
			...
			for ( ; $day <= $daysInMonth ; $day++) { 
				...
				if (in_array($month . $day, $magicNumbers)) {
					...
					if (isset($reasonCounts[$n])) $weight += $reasonCounts[$n];
		  ...
		  //       }
				...
				$n = nSum($dayCnt . $months . $days);
				if (in_array($n, $superSpecials) || in_array($n, $specials) || in_array($n, $masters) || str_replace($masters, "", $n) != $n)
				{
					$r = "nDCnt+MMDD: " . $n;
					$reason .= "<br>" . $r;
		   ...
		   //      	$reason .= "<br>YYMM=DD";
				...
				$weight = round($weight / $weightDiv);
				if ($weight > $weightMax) $weight = $weightMax;
				if ($weight > $weightBold) $reason = '<br><b>' . substr($reason, 4) . '</b>';
...
echo "<pre>";
print_r($reasonCounts);
print_r($becauseReasons);
die();
